...
function boardDueKind(t){
  var f = t.fecha_vencimiento;
  if(!f) return 'sin_fecha';
  var d = dayDiff(f);
  if(d<0 && t.estado!=='terminada') return 'vencido';
  if(d===0) return 'hoy';
  if(d<=7) return 'semana';
  return 'futuro';
}
function boardFilterTasks(tasks,p){
  ...
  return tasks.filter(function(t){
    ...
    if(f.entrega && boardDueKind(t)!==f.entrega) return false;
...
function operationalBoard(p,limit){
  ...
  var rows = tasks.map(function(t){
    ...
    return '<tr>'
      ...
      +'<td><div class="operational-actions"><button class="btn btns btnc op-action op-edit" title="Rápido: cambia estado, fecha, responsable y siguiente acción" aria-label="Edición rápida" onclick="A.quickEdit(\''+t.id+'\')"><span class="op-icon">'+iconHtml('zap')+'</span><span>Rápido</span></button><button class="btn btns btng op-action op-manage" title="Ficha: historial, avances, comentarios y seguimiento completo" aria-label="Ficha completa" onclick="A.manageTask(\''+t.id+'\')"><span class="op-icon">'+iconHtml('clipboard-list')+'</span><span>Ficha</span></button></div></td>'
      +'</tr>';
  }).join('');
